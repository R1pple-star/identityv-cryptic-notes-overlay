...
================================================================
纯逻辑状态机（零 Qt 依赖）。`ui/follow.py` 管「地图开着没」（G 开关），本模块管「地图动到哪了」——两者都在
...
## 尺度从哪来：两个「量出来的」源 > 对齐分
对齐分**选不出尺度**（分对 s 单调递增、真尺度处不是极小 ⇒ 挑分最小的尺度典型偏低
0.01~0.02、曲线平坦时偏低 0.09，且恒被下界截住）。两个实测源：
  1. **右侧导航列缩放滑条的圆点**（`vision.zoom_scale_from_roi`）—— 1ms，精度实测 ±0.01
     （实测误差 +0.000/+0.001/+0.008/−0.004）。**只在全屏布局可用**。
...
理由：「宁可什么都不给，也不给一张像是对的的假图」——粘在错位置的投影会被当成对
的位置用。**不 suspend 跟随**（地图还开着，跟随继续看着屏幕）。
"""
...
TRACK_BEAT_MIN_SEC = 1.5  # 跟踪心跳日志的最小间隔（每 tick 走哪条路的周期性输出）
...
def verdict(score_prev, score_new, ov_new):
    ...
    都是坏的，而且**错尺度往往分更低**（「缩模板骗分」——模板缩小 ⇒ 离群的不一致像素被
    一起缩掉 ⇒ 分必然更低。实测真尺度 0.42 时全局极小 0.086，换成错尺度 0.30 反而 0.126、
    图标偏 840px）。所以这里没有任何闸能把「尺度错」拦下来，只能靠调用方先把尺度钉住。
    """
