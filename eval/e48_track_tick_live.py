...
e33/e45 是复刻版（各自维护一份 `_track_tick` 逻辑副本），复刻与真代码可能脱节
（漏 import、入口早退等）：复刻全绿 ≠ 真代码能跑。本脚本直接驱动 app 真代码补上这层。

用真实截图（`captures/hotkey_20260919_1541xx.png`）喂真代码：播种走与热键同源的
`hint_icon` 图标锚定，之后逐帧调 `_maybe_track`，
...
def main():
    ...
    # ---- 3) 「按此种子对齐」（`_realign`）的真代码 ----
    # 验收思路 = 把匹配时的对齐方式（两段式）用在手动对齐上。旧行为两处病根：
    # ①拿的是上一次热键的**旧图**；②无锚点无尺度提示的全搜 ⇒ 缩模板骗分
    # （本批帧全搜得 s≈0.39 而真值 s≈0.83，分 0.038 照样过闸）。
    print("\n3) 手动重对齐（`_realign`）：旧图+全搜 vs 当场截图+两段式")
    # SEED 取这三帧真正的种子（三帧都匹配到「种子7 南-三缺一门[一楼]」）。
    win._seed = SEED
    win.floor_combo.setCurrentText(FLOOR)

    # 参照物 = **热键路径本尊**：`_two_stage_align(shot, 入口层结果, 真入口, 图标位)`。
    # 断言 `_realign` 在同一帧复现热键路径的位置 —— 即「匹配时的对齐方式用在手动对齐」。
    #
