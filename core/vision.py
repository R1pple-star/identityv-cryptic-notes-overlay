...
缩放滑条读数。引索路径、对齐、校准都依赖本模块。

面板坐标、模板路径、雾色等常量从 config.toml 读（缺配置/读失败兜底同值默认，绝不崩）。
"""
...
# 基准分辨率下的固定面板（x, y, 宽, 高）。依据标注参考图的地图边界白框标定。
# 其它分辨率经 panel_for_screen 等比外推或查 config [panel.rects] 校准表。
FIXED_PANEL = _CFG["panel_rect"]
_PANEL_REF_RES = _CFG["panel_res"]
# 匹配口径只有一种 ——「错配率」（5 类嵌 R^4 正单纯形 / 数一致 CCORR，见 to_match3 长注）。
# 旧「类号平方差」sqdiff 口径已删除（病理见 to_match3 长注；回滚走 git）。
...
def panel_for_screen(w: int, h: int):
    ...
    雾块 bbox 随探明状态漂移 dx −288~+480，暗块被游戏场景连通吞整屏。只能按分辨率静态适配。
    """
...
def detect_fog_panel(bgr_screen: np.ndarray):
    """按截屏物理尺寸适配地图面板。返回 (x, y, w, h) 或 None。"""
...
def walls_as_floors(cls, region):
    ...
    墙类不进对齐层匹配的原因是厚度惩罚（游戏内墙 5-9px vs 参考墙 1-3px，直进重合分
    0.046→1.31，参考侧膨胀也救不回）；图标锚定下两侧墙线画位重合 60-67%（素材与
    游戏内同渲染器）。并回旧语义后对齐层匹配行为不变；**cls5 原样保留**，由入口层的「墙重合度量」
...
# 实测（真种子+真尺度+锚定）后果：真对齐下类别一致率 87%~92%，
...
def _find_icon(bgr, px, py, pw, ph,
    ...
    （core.entrance._crop_around_icon 按 k 缩放裁样），必须量准；粗网格（下限 0.5）会把
    常见小图标钉在网格底（真 k 实测 0.40-0.45，网格细化后 NCC 0.64→0.81+）。
    ...
    if not icon_path.exists():
        return None, 0, None  # 缺图标模板不检测（模板由 assets/_icon_entrance.png 提供）
...
def follow_features(region):
    ...
    关态下技能特效实测可把 content 冲到 0.31）；
    fog    = FOG_BGR tol24 色距占比（开态 0.20~0.69；游戏世界最高 0.17）；
    struct = (cls2房间|cls3通路|cls5墙) 占比——不参与开合判定，仅日志：游戏世界
    实测能到 0.21，任何低于此的阈值都会被跨过而误判为「开」（见 NAV_BAND 长注）。
    开合判定走 map_is_open / map_open_from_roi（侧栏导航列 NCC 为主判据）。
    """
...
#   窗口化那档**不能用邻域搜索救**：±40/±30 搜索把窗口化正样本从 0.36 抬到 0.664，
#   但真负样本同时被抬到 0.271，分离带反而更窄 ⇒ **不做位置搜索**。
...
def map_roi(panel=FIXED_PANEL):
    ...
    最缩小态（见 NAV_DOT_RANGE 长注）。只加长 ROI，NAV_BAND 本身不动。
    """
    px, py, pw, ph = panel
...
def map_open_from_roi(roi_bgr, panel=FIXED_PANEL):
    ...
    它能把开态救回来，代价是**关闭动画那几帧也会被判成开**（导航列已消失而残雾仍 ≥ 阈值）。
    跟踪链路必须区分这两者 —— 只有主判据说开时，判丢才允许销毁 `_last_rgba`，
    否则 G 开回来无图可放回。
    """
...
def map_is_open(shot_bgr, panel=FIXED_PANEL):
    ...
    热键路径在跑匹配前用它拒掉大厅/结算等非地图画面。
    """
    is_open, why, _nav_ok = map_open_from_roi(roi_of(shot_bgr, panel), panel)
    return is_open, why


# ---- 缩放滑条读数（导航列的长条 + 圆点，点的位置=缩放程度）----------------------------
# 动机：**对齐分选不出尺度** —— 分对 s 单调递增、真尺度处不是极小，于是「挑分
...
# 峰。模板（多张全屏实机图的均值）轨道列 x=84，圆点活动范围 y∈[315,805]。
# ⚠️ 窗口必须覆盖到 y≈805：圆点能走到 **y=752**（最缩小态，真尺度 0.83），窗口截短的话
# 最缩小态**读数直接失败**（峰-基只剩 4，过不了闸）⇒ 跟随拿旧尺度做全平移会错位
# （实测图标偏 184px）。轨道实体延伸到 y≈790，再往下是底部圆钮（全览/我的位置，
# y≥860）——别把窗开过头。
...
# 标定两类真值：
#   **图标锚点法**（图标误差 1~5px —— 引索 json 的图标 cx/cy 经 M 映到 `_find_icon`
...
def zoom_scale_from_roi(roi_bgr, panel=FIXED_PANEL):
    ...
    **调用方必须能回退**（如图标尺子、就地判丢——错尺度上搜索会锁到错位置）。
    """
    ncc = nav_column_ncc(roi_bgr, panel)
