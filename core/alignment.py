...
# 对齐(投影重合)专用尺度域：上界放宽到 1.5。s = ref像素/面板像素，实测视图 0.75(整图
# 适配)~0.99(放大)；游戏内还能继续放大，17.13 放大 8% 就要 s=1.065，超原上界 1.0 会让
# 对齐滑到边界判不可靠。种子匹配仍用 SCALES(观测域即可，多搜只增假阳风险)。
ALIGN_SCALES = tuple(np.round(np.arange(0.40, 1.501, 0.05), 2))
